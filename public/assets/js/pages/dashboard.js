...
var KTDashboard = function() {
    ...
    // Latest Orders
    var MaximumItemStockLevel = function() {
        if ($('#kt_datatable_latest_orders').length === 0) {
            return;
        }
        var data_url = $('#kt_datatable_latest_orders').attr('data-url');
        var dataJSONArray = [
                {
                    "Name":"Blue Riband",
                    "ArabicName":"بلو ريباند",
                    "MaxQty":"190",
                    "StockQty":"500",
                    "Diff":'210',
                    "Actions":null
        ...
        var datatable = $('.kt-datatable').KTDatatable({
            data: {
               ...
               // type: 'remote',
                /*source: {
                    read: {
                        url: data_url,
                        headers: {'name': 'csrf-token', 'content': ''},
                        map: function(raw) {
                            // sample data mapping
                            var dataSet = raw;
            ...
            columns: [{
                    ...
                    var output = '\
                        <div class="kt-user-card-v2">\
                            ...
                            <div class="kt-user-card-v2__details">\
                                ...
                                <span class="kt-user-card-v2__email text-muted fz-16">'
                        + data.ArabicName + '</span>\
                            </div>\
                        </div>';
                ...
                template: function(data) {
                    return '<span class="kt-font-bold">' + data.MaxQty + '</span>';
                }
            }, {
                field: "StockQty",
                title: "Stock Qty",
                width: 100,
                template: function(data) {
                    return '<span class="kt-font-bold">' + data.StockQty + '</span>';
                }
            }, {
                field: "Diff",
                title: "Diff. Qty",
                ...
                template: function(data, row) {
                    ...
                    return '<span class="btn btn-bold btn-sm btn-font-sm ' + status[3].class + '">' +
                        data.Diff + '</span>';
                }
            },
                {
    ...
    var MinimumItemStockLevel = function() {
        if ($('#minimum_item_stock_level').length === 0) {
            return;
        }
       // var data_url = $('#kt_datatable_latest_orders').attr('data-url');
        var dataJSONArray = [
        ...
        var datatable = $('.kt-datatable2').KTDatatable({
            ...
            columns: [{
                ...
                template: function(data, row) {
                    ...
                    return '<span class="btn btn-bold btn-sm btn-font-sm ' + status[6].class + '">' +
                        data.Diff + '</span>';
                }
            },
                {
    ...
    return {
        // Init demos
        init: function() {
            MaximumItemStockLevel();
            MinimumItemStockLevel();
            // init charts
            dailySales();
            profitShare();
            salesStats();
