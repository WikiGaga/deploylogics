var id_01 = 'supplier_account_sr_number';
var id_02 = 'supplier_bank_name';
var id_03 = 'supplier_bank_account_no';
var id_04 = 'supplier_bank_account_title';
var id_05 = 'supplier_bank_iban_no';
function addDatasm(){
    $('#addDatasm').click(function(){
        ...
        var tr = '';
        var bankArray = [];
        $('#supplier_bank_name>option').each(function(){
            bankArray.push({
                id: $( this ).val(),
                name: $( this ).text(),
            });
        ...
        tr += '<tr>' +
            ...
            '<input type="text" value="'+total_length+'" name="pdsm['+total_length+']['+id_01+']"  class="form-control erp-form-control-sm handle" readonly></td>' +
            '<td>'+
                '<select  name="pdsm['+total_length+']['+id_02+']" class="form-control supplier_bank_name erp-form-control-sm moveIndexsm">';
                    for(var i=0;bankArray.length>i;i++){
                        tr+='<option value='+bankArray[i]['id']+' '+(val_02 == bankArray[i]['id']?"selected":"")+'>'+bankArray[i]['name']+'</option>';
                    }
            tr+='</select>'+
            '</td>' +
            '<td><input type="text" name="pdsm['+total_length+']['+id_03+']" value="'+val_03+'" title="'+val_03+'" class=" moveIndexsm form-control erp-form-control-sm"></td>' +
            '<td><input type="text" name="pdsm['+total_length+']['+id_04+']" value="'+val_04+'" title="'+val_04+'" class="moveIndexsm form-control erp-form-control-sm" ></td>' +
            '<td><input type="text" name="pdsm['+total_length+']['+id_05+']" value="'+val_05+'" title="'+val_05+'" class="moveIndexsm form-control erp-form-control-sm" ></td>' +
...
function updateKeyssm(){
    ...
    if(total_length != 0){
        for(var i=0;total_length > i; i++){
            ...
            $('#repeated_datasm tr:eq('+i+') td:eq(4)').find('input').attr('name','pdsm['+j+']['+id_05+']');
            }
    }
}
function dataEditsm(){
