...
$(document).on('click' , '#addData,#addDataSubCustomer' , function(e){
    event.stopPropagation();
    event.stopImmediatePropagation();
    // Only Mouse Click and Enter Key is accepted
    if($(this).data('type') == "person"){
        if (typeof arr_text_Field !== 'undefined'){
            text_Fields = arr_text_Field;
        }else{
            text_Fields = [];
        }
        if (typeof arr_hidden_field !== 'undefined'){
            hidden_fields = arr_hidden_field;
        }else{
            hidden_fields = [];
        }
    }
    if($(this).data('type') == "subcustomer"){
        if (typeof arr_customer_text_Field !== 'undefined'){
            text_Fields = arr_customer_text_Field;
        }else{
            text_Fields = [];
        }
        if (typeof arr_customer_hidden_field !== 'undefined'){
            hidden_fields = arr_customer_hidden_field;
        }else{
            hidden_fields = [];
        }
    }
...
function addRowData(thix){
    ...
    if(validator){
        ...
        for(var i=0;i<text_Fields.length;i++){
            if(text_Fields[i]['type'] == 'select'){
                ...
                if(text_Fields[i]['convertType'] == 'input'){
                    ...
                    var selected_val = thix.closest('tr').find('select#'+name).val();
        ...
        for(var i=0; arrSelect.length > i; i++){
            ...
            thix.closest('.table.erp_form__grid').find('.erp_form__grid_body>tr:last-child').find('td:eq('+arrSelect[i]['index']+')>.erp-select2>select').attr('name', prefix+'['+total_length+']['+name+']');
