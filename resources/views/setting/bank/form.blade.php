...
@section('content')
    @php
        ...
        if($case == 'new'){}
        if($case == 'edit'){
            $id = $data['current']->bank_id;
            $name = $data['current']->bank_name;
            $status = $data['current']->bank_entry_status;
        }
    @endphp
    @permission($data['permission'])
    <form id="bank_form" class="master_form kt-form" method="post" action="{{ action('Setting\BankController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="bank_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="bank_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
