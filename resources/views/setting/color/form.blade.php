...
@section('content')
    @php
        ...
        if($case == 'new'){}
        if($case == 'edit'){
            $id = $data['current']->color_id;
            $name = $data['current']->color_name;
            $status = $data['current']->color_entry_status;
        }
    @endphp
    @permission($data['permission'])
    <form id="city_form" class="master_form kt-form" method="post" action="{{ action('Setting\ColorController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="color_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="color_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
