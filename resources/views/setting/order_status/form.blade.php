...
@section('content')
    @php
        ...
        if($case == 'new'){}
        if($case == 'edit'){
            $id = $data['current']->order_status_id;
            $name = $data['current']->order_status_names;
            $arabic_name = $data['current']->order_status_arabic_names;
            $serial = $data['current']->serial;
            $status = $data['current']->order_status_entry_status;
            $default = $data['current']->order_status_default_status;
        }
    @endphp
    @permission($data['permission'])
    <form id="order_status_form" class="master_form kt-form" method="post" action="{{ action('Setting\OrderStatusController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                ...
                <div class="kt-portlet__body">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group-block row">
                            <label class="col-lg-3 erp-col-form-label">Status Names: <span class="required">*</span></label>
                            <div class="col-lg-6">
                                <input type="text" name="name" value="{{isset($name)?$name:""}}" class="form-control erp-form-control-sm medium_text">
                            </div>
                        </div>
                        <div class="form-group-block row">
                            <label class="col-lg-3 erp-col-form-label">Arabic Names:</label>
                            <div class="col-lg-6">
