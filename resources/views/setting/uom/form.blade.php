...
@section('content')
    @php
        ...
        if($case == 'new'){}
        if($case == 'edit'){
            $id = $data['current']->uom_id;
            $name = $data['current']->uom_name;
            $status = $data['current']->uom_entry_status;
        }
    @endphp
    @permission($data['permission'])
    <form id="uom_form" class="master_form kt-form" method="post" action="{{ action('Setting\UomController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="uom_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="uom_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
