...
@section('content')
    ...
    @if($data['form_type'] == 'pos-sales-invoice')
        @php $form_type = 'pos_sale_invoice'; @endphp
    ...
    @endif
    @if($data['form_type'] == 'sales-invoice' && !isset($data['sales_invoice_noauth']))
           @php $form_type = 'sale_invoice'; @endphp
           <form id="sales_form" class="kt-form" method="post" action="{{ route('sales_invoice', isset($id)?$id:"") }}">
           <input type="hidden" value='{{$form_type}}' id="form_type" name="form_type">
    @endif
    @if($data['form_type'] == 'sales-invoice' && isset($data['sales_invoice_noauth']))
           @php $form_type = 'sale_invoice'; @endphp
           <form id="sales_form" class="kt-form" method="post" action="{{ route('sales_invoice_noauth', isset($id)?$id:"") }}">
           <input type="hidden" value='{{$form_type}}' id="form_type" name="form_type">
    @endif
    @if($data['form_type'] == 'sales-invoice-c')
           @php $form_type = 'sale_invoice-c'; @endphp
        ...
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ...
                <div class="row form-group-block">
                    ...
                    <div class="col-lg-8">
                        <div class="row">
                        <label class="col-lg-3 erp-col-form-label">Sales Quotation:</label>
                            <div class="col-lg-9">
                                <div class="erp_form___block">
                                    <div class="input-group open-modal-group">
                                        <div class="input-group-prepend">
                                        ...
                                        <input type="text" value="{{isset($sales_quotation_code)?$sales_quotation_code:''}}" data-url="{{action('Common\DataTableController@inlineHelpOpen','salesQuotationHelp')}}" id="sales_quotation_code" name="sales_quotation_code" class="open_inline__help form-control erp-form-control-sm moveIndex" placeholder="Enter Sales Quotation">
                                        <input type="hidden" id="sales_quotation_id" name="sales_quotation_id" value="{{isset($sales_quotation_id)?$sales_quotation_id:''}}"/>
                                        <div class="input-group-append">
                                            <span class="input-group-text group-input-btn" id="getStockQuotationData">
                                                GO
                                            </span>
                                        </div>
                                    </div>
...
@section('customJS')
    ...
    <script>
        // Stock Qutation Data
        $('#getStockQuotationData').click(function(){
            var thix = $(this);
            var val = thix.parents('.input-group').find('input#sales_quotation_id').val();
            if(val){
                swal.fire({
                    title: 'Alert!',
