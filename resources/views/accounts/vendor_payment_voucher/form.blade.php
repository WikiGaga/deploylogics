...
@php
    ...
    if($case == 'edit'){
        ...
        $voucher_debit_amount = number_format($data['current']->voucher_debit,3);
        ...
        $voucher_purchase_bills = $data['current_purchase'];
        $voucher_purchase_return_bills = $data['current_purchase_return'];
...
<!--begin::Form-->
<form id="voucher_form" class="kt-form" method="post" action="{{action('Accounts\VoucherController@vpvstore', [$type,isset($id)?$id:''])}}">
    @csrf
        ...
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ...
                <div class="form-group-block row">
                    ...
                    <div class="col-lg-3">
                        <div class="row">
                            ...
                            <div class="col-lg-7">
                                <input type="text" id="bill_total_amount" name="bill_total_amount" value="{{$voucher_debit_amount}}" autocomplete="off" class="validNumber validOnlyFloatNumber form-control erp-form-control-sm {{$on_account_voucher == 1?"":"readonly"}}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <ul class="employee-tab-nav nav nav-tabs nav-tabs-line nav-tabs-bold nav-tabs-line-3x nav-tabs-line-success" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" data-toggle="tab" href="#tab_1" role="tab">Purchase (GRN)</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" data-toggle="tab" href="#tab_2" role="tab">Purchase Return</a>
                    </li>
                </ul>
                <div class="tab-content">
                                        ...
                                        <div class="table-scroll form_input__block">
                                            <table id="grnVoucherDtls" data-prefix="inv" class="table erp_form__grid erp_form__grid_th_resize table-resizable dtr-inline {{$on_account_voucher == 1?" pointerEventsNone":""}}">
                                                ...
                                                <tr>
                                                    ...
                                                    <th scope="col">
                                                        <div class="erp_form__grid_th_title">Vendor Bill No.</div>
                                                    </th>
                                                    <th scope="col">
                                                ...
                                                @if($on_account_voucher == 0)
                                                    @foreach($voucher_purchase_bills as $bill)
                                                        @php
                                                            $bi = $loop->iteration;
                                                           // dump($bill);
                                                        ...
                                                        @if(strtolower($grn_type) == 'grn')
                                                            <tr>
                                                                <td>
                                                                    ...
                                                                    <input readonly type="hidden" value="{{$grn_type}}" name="inv[{{$bi}}][grn_type]" data-id="grn_type" class="form-control erp-form-control-sm">
                                                                </td>
                                                                <td>
                                                ...
                                                <tr style="border-top: 2px solid #007df6;border-bottom: 2px solid #007df6;">
                                                    ...
                                                    <td></td>
                                                    <td>
                                                        <input type="text" readonly value="{{$voucher_debit_amount}}" id="total_current_amount" name="total_current_amount" class="total_current_amount validNumber validOnlyFloatNumber form-control erp-form-control-sm">
                                                    </td>
                                                    <td></td>
                    ...
                    <div class="tab-pane" id="tab_2" role="tabpanel">
                        @include('accounts.vendor_payment_voucher.purchase_return')
                    </div>{{-- Contact detail--}}
                </div>
                                    ...
                                    @if(isset($data['deduction']) && count($data['deduction']) != 0)
                                        @foreach($data['deduction'] as $duc)
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text" class="amount validNumber tb_moveIndex validOnlyFloatNumber form-control erp-form-control-sm" name="duc[{{$loop->iteration}}][amount]" value="{{number_format($duc['voucher_credit'],3)}}" data-id="amount">
                                                </td>
                                                <td class="text-center">
                                ...
                                @if(isset($dtls))
                                    @foreach($dtls as $row)
                                        ...
                                        <tr>
                                            ...
                                            @php $credit = $row->voucher_credit; $fc_credit = $row->voucher_fc_credit; @endphp
                                            <td><input type="text" data-id="supplier_bank" name="pd[{{ $loop->iteration }}][supplier_bank]" value="{{$row->payee_bank}}" class="supplier_bank form-control erp-form-control-sm readonly" readonly></td>
    ...
    <script>
        ...
        function getDatafromGrnBill(arr){
            ...
            if(validate){
                ...
                var url = '/accounts/get-inv-detail';
                $.ajax({
                    headers: {
                        ...
                        if(!valueEmpty(response['data']['list'])){
                            ...
                            list.forEach(function(item,index){
                                ...
                                tr += '<tr>' +
                                    ...
                                    '<input readonly type="hidden" value="'+item.grn_type+'" name="inv['+i+'][grn_type]" data-id="grn_type" class="form-control erp-form-control-sm"> ' +
                            ...
                            var tbody = $('#grnVoucherReturn').find('tbody.erp_form__grid_body');
                            tbody.html("");
                            var list = response['data']['list_pr'];
                            var tr = "";
                            var i = 0;
                            list.forEach(function(item,index){
                                var grn_date = !valueEmpty(item.grn_date)?item.grn_date:'';
                                ...
                                tr += '<tr>' +
                                    '<td>' +
                                    '<input readonly type="text" value="'+i+'" name="invr['+i+'][sr_no]" data-id="sr_no" class="form-control erp-form-control-sm">' +
                                    '<input readonly type="hidden" value="'+item.branch_id+'" name="invr['+i+'][branch_id]" data-id="branch_id" class="form-control erp-form-control-sm">' +
                                    '<input readonly type="hidden" value="'+item.grn_id+'" name="invr['+i+'][grn_id]" data-id="grn_id" class="form-control erp-form-control-sm"> ' +
                                    '<input readonly type="hidden" value="'+item.grn_type+'" name="invr['+i+'][grn_type]" data-id="grn_type" class="form-control erp-form-control-sm"> ' +
                                    '</td>' +
                                    '<td> <input readonly type="text" value="'+item.branch_name+'" name="invr['+i+'][branch_name]" data-id="branch_name" class="form-control erp-form-control-sm"> </td>' +
                                    '<td> <input readonly type="text" value="'+item.grn_code+'" name="invr['+i+'][grn_code]" data-id="grn_code" class="form-control erp-form-control-sm"> </td>' +
                                    '<td> <input readonly type="text" value="'+grn_date+'" name="invr['+i+'][grn_date]" data-id="grn_date" class="form-control erp-form-control-sm"> </td>' +
                                    '<td> <input readonly type="text" value="'+grn_total_net_amount+'" name="invr['+i+'][grn_total_net_amount]" data-id="grn_total_net_amount" class="validNumber validOnlyFloatNumber form-control erp-form-control-sm"> </td>' +
                                    '<td> <div class="input-group"> <input readonly type="text" value="" name="invr['+i+'][ref_code]" data-id="ref_code" class="ref_code form-control erp-form-control-sm readonly"><div class="input-group-prepend"><span class="input-group-text btn-ref-code-selected"><i class="la la-refresh"></i></span></div> </div></td>' +
                                    '</tr>';
                            })
                            tbody.html(tr);
        ...
        $(document).on('click','#grnVoucherDtls input[data-id="grn_code"]',function(){
            $(this).select();
            var val = $(this).val();
            $('.selected_ref_code').val(val)
            document.execCommand("copy");
            toastr.success(val + ' selected for Purchase Return');
        });

        $(document).on('click','.btn-ref-code-remove',function(){
            $('.selected_ref_code').val("")
        });

        $(document).on('click','#grnVoucherReturn .btn-ref-code-selected',function(){
            var ref_code =  $('.selected_ref_code').val();
            ref_code = ref_code.trim();
            var thix = $(this);
            var thix_ref_code = thix.parents('.input-group').find('.ref_code')
            var old_ref_code = thix_ref_code.val();
            thix_ref_code.val(ref_code);
            var pr_amount = $(this).parents('tr').find('input[data-id="grn_total_net_amount"]').val();
            pr_amount = pr_amount.replace(',', '');
            $('#grnVoucherDtls tbody tr').each(function(){
                var grn_code = $(this).find('input[data-id="grn_code"]').val();
                if(grn_code == ref_code || grn_code == old_ref_code){
                    thix_ref_code.css("background","#deb887");
                    var paid_amount = $(this).find('input[data-id="paid_amount"]').val();
                    paid_amount = paid_amount.replace(',', '');
                    if(grn_code == old_ref_code){
                        paid_amount = parseFloat(paid_amount) - parseFloat(pr_amount);
                    }
                    if(grn_code == ref_code){
                        paid_amount = parseFloat(paid_amount) + parseFloat(pr_amount);
                    }
                    $(this).find('input[data-id="paid_amount"]').val(parseFloat(paid_amount).toFixed(3));

                    var net_amount = $(this).find('input[data-id="grn_total_net_amount"]').val();
                    net_amount = net_amount.replace(',', '');
                    var balance_amount = parseFloat(net_amount) - parseFloat(paid_amount);
                    $(this).find('input[data-id="balance_amount"]').val(parseFloat(balance_amount).toFixed(3));

                    var current_amount = $(this).find('input[data-id="current_amount"]').val();
                    current_amount = current_amount.replace(',', '');
                    var current_balance = parseFloat(balance_amount) - parseFloat(current_amount);
                    $(this).find('input[data-id="current_balance"]').val(parseFloat(current_balance).toFixed(3));
                    setInterval(function(){thix_ref_code.css("background", "");},1500);
