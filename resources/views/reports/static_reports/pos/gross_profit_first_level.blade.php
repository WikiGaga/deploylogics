...
//dd($qry);
                    ...
                    <table width="100%" id="gross_profit" class="static_report_table table bt-datatable table-bordered">
                        ...
                        @foreach($getdata as $row)
                            <tr data-id="{{$row->group_item_parent_id.'@'.$supplier_ids.'@'.$product_ids.'@'.$product_groups}}">
                                <td>{{$ki}}</td>
        ...
        $(document).on('click','td',function(){
            ...
            if(tbody.length == 1){
                ...
                if(validate && xhrGetData){
                    ...
                    xhrGetData = false;
                    var formData = {
                        report_case : 'gross_profit_last_level',
                        report_type: 'static',
                    ...
                    var url = "{{ action('Report\UserReportsController@staticStore', ['static','gross_profit_last_level','']) }}";
                    $.ajax({
                        headers: {
