                    ...
                    if(count($data['product_group']) != 0){
                        ...
                        $qry = "SELECT
                            ...
                            CASE 
                                ...
                                THEN ROUND(SUM(SALE_AMOUNT) / ROUND(SUM(SALE_QTY) + SUM(SALE_RET_QTY),0) ,0) 
                            END  AVG_SALE_RATE ,
                            ROUND(SUM(SALE_AMOUNT),0) SALE_AMOUNT,
                            ROUND(SUM(COST_AMOUNT),0) COST_AMOUNT,
                            CASE 
                                WHEN ROUND(SUM(SALE_QTY) + SUM(SALE_RET_QTY),0) > 0
                                THEN  ROUND(SUM(NET_GP) / ROUND(SUM(SALE_QTY) + SUM(SALE_RET_QTY),0) ,2)  
                            END PER_ITEM_GP ,
                            ROUND(SUM(NET_GP),0)  NET_GP ,
                            CASE 
                                WHEN SUM(SALE_AMOUNT) > 0 
                                THEN  ROUND( SUM(NET_GP)  /  SUM(SALE_AMOUNT) * 100 ,2) 
                            END  GP_PER
                        FROM(
                            select
...
//dd($qry);
                    ...
                    <table width="100%" id="gross_profit" class="static_report_table table bt-datatable table-bordered">
                        ...
                        @foreach($getdata as $row)
                            <tr data-id="{{$row->branch_id.'@'.$supplier_ids.'@'.$product_ids.'@'.$product_groups}}">
                                <td>{{$ki}}</td>
...
@section('customJS')
    <script>
        ...
        $(document).on('click','td',function(){
            ...
            var product_groups = myArray[3].split(",");
            if(valueEmpty(product_groups)){
                product_groups = '';
            }
            ...
            if(tbody.length == 1){
                ...
                if(validate && xhrGetData){
                    ...
                    xhrGetData = false;
                    var formData = {
                        report_case : 'gross_profit_first_level',
                        report_type: 'static',
                        ...
                        'report_product_groups' : product_groups,
                    };
                    var url = "{{ action('Report\UserReportsController@staticStore', ['static','gross_profit_first_level','']) }}";
                    $.ajax({
                        headers: {
