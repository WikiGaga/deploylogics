...
@section('title', 'Vendor Wise Purchase Summary')
            ...
            <div class="row row-block">
                <div class="col-lg-12">
                    @php
                        ...
                        $qry = "SELECT  SUPPLIER_ID ,
                                    SUPPLIER_NAME  ,
                                    GRN_TYPE ,
                                    COUNT(distinct GRN_ID)  INV_NUMBERS ,
                                    SUM(GRN_TOTAL_QTY)  GRN_TOTAL_QTY ,
                                    SUM(GRN_TOTAL_AMOUNT)  GRN_TOTAL_AMOUNT  ,
                                    SUM(GRN_TOTAL_DISC_AMOUNT)  GRN_TOTAL_DISC_AMOUNT ,
                                    SUM(GRN_TOTAL_GST_AMOUNT) GRN_TOTAL_GST_AMOUNT ,
                                    SUM(GRN_TOTAL_FED_AMOUNT)  GRN_TOTAL_FED_AMOUNT,
                                    SUM(GRN_TOTAL_SPEC_DISC_AMOUNT) GRN_TOTAL_SPEC_DISC_AMOUNT  ,
                                    SUM(GRN_OVERALL_DISC_AMOUNT) GRN_OVERALL_DISC_AMOUNT  ,
                                    SUM(GRN_ADVANCE_TAX_AMOUNT)   GRN_ADVANCE_TAX_AMOUNT ,
                                    SUM(GRN_TOTAL_NET_AMOUNT) GRN_TOTAL_NET_AMOUNT
                                    FROM
                                    (
                                    select distinct
                                    SUPPLIER_ID ,
                                    ...
                                    GRN_TOTAL_QTY ,
                                    GRN_TOTAL_AMOUNT ,
                                    GRN_TOTAL_DISC_AMOUNT ,
                                    GRN_TOTAL_GST_AMOUNT  ,
                                    GRN_TOTAL_FED_AMOUNT ,
                                    GRN_TOTAL_SPEC_DISC_AMOUNT ,
                                    GRN_OVERALL_DISC_AMOUNT ,
                                    GRN_ADVANCE_TAX_AMOUNT  ,
                    ...
                    <table width="100%" id="rep_sale_invoice_datatable" class="static_report_table table bt-datatable table-bordered">
                        ...
                        <tr><td colspan="11"><h6>Purchase Invoice</h6></td></tr>
                        @foreach($pi as $key=>$product)
                            <tr>
                                <td>{{$ki}}</td>
                                <td>{{$product->supplier_name}}</td>
                                <td class="text-right">{{$product->inv_numbers}}</td>
                            ...
                            @php
                                ...
                                $pi_no_of_invoices += $product->inv_numbers;
                            @endphp
                        @endforeach
                        <tr class="sub_total">
                            <td colspan="2" class="rep-font-bold">Purchase Invoice Total:</td>
                            <td class="text-right rep-font-bold">{{number_format($pi_no_of_invoices,3)}}</td>
                        ...
                        @if(isset($data['specific_purchase_type']) && in_array($data['specific_purchase_type'],['pr','all']) )
                            <tr><td colspan="11"><h6>Purchase Return</h6></td></tr>
                            @foreach($pr as $key=>$item)
                                <tr>
                                    <td>{{$pr_key}}</td>
                                    <td>{{$item->supplier_name}}</td>
                                    <td class="text-right">{{$item->inv_numbers}}</td>
                                    <td class="text-right">{{number_format($item->grn_total_amount)}}</td>
                                ...
                                @php
                                    ...
                                    $pr_no_of_invoices += $item->inv_numbers;
                                @endphp
                            @endforeach
                            <tr class="sub_total">
                                <td colspan="2" class="rep-font-bold">Purchase Return Total:</td>
                                <td class="text-right rep-font-bold">{{number_format($pr_no_of_invoices)}}</td>
                        ...
                        @php
                            $grand_no_of_invoices = $pi_no_of_invoices - $pr_no_of_invoices;
                        ...
                        <tr class="grand_total">
                            ...
                            <td class="text-right rep-font-bold">{{number_format($grand_no_of_invoices,2)}}</td>
