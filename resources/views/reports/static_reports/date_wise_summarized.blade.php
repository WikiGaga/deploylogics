...
@section('content')
    ...
    <div class="kt-portlet" id="kt_portlet_table">
        <div class="kt-portlet__head">
            <div class="kt-invoice__brand">
                ...
                <h6 class="kt-invoice__criteria">
                    ...
                    <span
                        style="color: #5578eb;">{{ ' ' . date('d-m-Y', strtotime($data['from_date'])) . ' to ' . date('d-m-Y', strtotime($data['to_date'])) . ' ' }}</span>
                </h6>
                ...
                @if (!empty($data['chart_account_id']))
                    @php
                        $chart_detail = DB::table('tbl_acco_chart_account')
                            ->select('chart_code', 'chart_name')
                            ->where('chart_account_id', $data['chart_account_id'])
                            ->first();
        ...
        @php
            $where = '';
            $where .=
                " and voucher_date between to_date ('" .
                $data['from_date'] .
            ...
            if (!empty($data['chart_account_id'])) {
                $chart_lists = DB::table('tbl_acco_chart_account')
                    ->select('chart_code', 'chart_name')
                    ->where('chart_account_id', $data['chart_account_id'])
                    ->first();
            ...
            from
                VW_ACCO_VOUCHER
            where posted = 1 AND BRANCH_ID IN (" .
                implode(',', $data['branch_ids']) .
                ")
                $where
