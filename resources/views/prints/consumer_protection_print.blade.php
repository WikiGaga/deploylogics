@php
    ...
    $delivery_no = $data['current']->protection_delivery_id;
    ...
    $days = $data['current']->protection_credit_days;
    $mobile = $data['current']->protection_mobile_no;
    $notes = $data['current']->protection_remarks;
    $NetTotal = $data['current']->protection_net_amount;
                    ...
                    @endphp
                        @if(isset($dtls))
                            @foreach($dtls as $data)
                               ...
                               // dd($data);
                                    $i++;
                                    $totAmt += $data->protection_dtl_amount;
                                    $totDiscAmt += $data->protection_dtl_disc_amount;
                                    $totVatAmt += $data->protection_dtl_vat_amount;
                                    $totGrossAmt += $data->protection_dtl_total_amount;
                                @endphp
                                <tr>
                                    <td class="dtl-contents aligncenter">{{ $loop->iteration }}</td>
                                    <td class="dtl-contents alignleft">{{$data->protection_dtl_barcode}}</td>
                                    ...
                                    <td class="dtl-contents aligncenter">{{$data->protection_dtl_quantity}}</td>
                                    <td class="dtl-contents alignright">{{number_format($data->protection_dtl_rate,3)}}</td>
                                    <td class="dtl-contents alignright">{{number_format($data->protection_dtl_amount,3)}}</td>
                                    {{--<td class="dtl-contents alignright">{{number_format($data->protection_dtl_disc_amount,3)}}</td>--}}
                                    <td class="dtl-contents alignright">{{number_format($data->protection_dtl_vat_amount,3)}}</td>
                                    <td class="dtl-contents alignright">{{number_format($data->protection_dtl_total_amount,3)}}</td>
                                </tr>
                            @endforeach
                        @endif
                        @if($i<=8)
                                ...
                                @endphp
                                    @if(isset($Expdtls))
                                        @foreach($Expdtls as $expense)
                                            @php
                                                $totExpAmt += $expense->protection_expense_amount;
                                            @endphp
                                            <tr>
                                                <td width="60%" class="heading alignleft">{{$expense->accounts->chart_name}}</td>
                                                <td width="40%" class="heading alignright">{{number_format($expense->protection_expense_amount,3)}}</td>
                                            </tr>
                                        @endforeach
                                    @endif
                                    @php
