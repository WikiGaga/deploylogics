            ...
            @if(isset($chq_dtls))
                @foreach($chq_dtls as $row)
                    @php
                        ...
                        $PayeeTitle = \Illuminate\Support\Facades\DB::table('tbl_purc_supplier_account')
                            ->where('supplier_account_id',$row->tbl_supplier_account_id)
                            ->where('supplier_account_no',$row->payee_ac_no)
                            ->first('supplier_account_title');
                    ...
                    <tr>
                        ...
                        <td class="dtl-contents aligncenter">{{isset($PayeeTitle->supplier_account_title)?$PayeeTitle->supplier_account_title:''}}</td>
                        <td class="dtl-contents aligncenter">{{isset($row->payee_ac_no)?$row->payee_ac_no:''}}</td>
                        <td class="dtl-contents aligncenter">{{isset($row->payment_mode->payment_term_name)?$row->payment_mode->payment_term_name:''}}</td>
                        <td class="dtl-contents aligncenter">{{isset($row->payee_bank)?$row->payee_bank:''}}</td>
    ...
    <table class="tabData">
        ...
        <tbody>
            @php
                ...
                $sum_voucher_bill_paid_amount = 0;
            ...
            @foreach($voucher_bills as $bill)
                @php
                    ...
                    $sum_voucher_bill_paid_amount += $bill->voucher_bill_paid_amount;
            ...
            <tr>
                ...
                <td class="dtl-head alignright">{{number_format($sum_voucher_bill_paid_amount,3)}}</td>
