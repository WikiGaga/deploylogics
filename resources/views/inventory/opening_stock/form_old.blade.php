...
@section('content')
    ...
    @php
            ...
            $type =$data['form_type'];
    @endphp
    <form id="opening_stock_form" class="master_form kt-form" method="post" action="{{ action('Inventory\StockController@store', [$type,$id]) }}">
    @csrf
            ...
            <div class="kt-portlet__body">
                    <div class="kt-portlet__body">
                                    ...
                                    <div class="dropdown dropdown-inline">
                                        ...
                                        @php
                                            $headings = ['Sr No','Barcode','Product Name','UOM','Packing','Qty',
                                                          'Pur Rate','Production Date','Expiry Date','Batch No','Amount'];
                                        @endphp
                        ...
                        <div class="form-group-block" style="overflow: auto;">
                            <table id="OpeningStockForm" class="ErpForm table table-striped table-responsive table-bordered table-hover table-checkable no-footer dtr-inline collapsed table-resizable">
                                <thead>
                                <tr>
                                    <th width="5%">Sr No</th>
                                ...
                                <tr id="dataEntryForm">
                                    ...
                                    <td><input readonly type="text" id="production_date" class="form-control form-control-sm c-date-p kt_datepicker_3" value="{{date('d-m-Y')}}" title="{{date('d-m-Y')}}" /></td>
                                    <td><input readonly type="text" id="expiry_date" class="form-control form-control-sm c-date-p kt_datepicker_3" value="{{date('d-m-Y')}}" title="{{date('d-m-Y')}}" /></td>
                                    ...
                                    @if(isset($dtls))
                                        @foreach($dtls as $dtl)
                                            <tr>
                                                ...
                                                <td>
                                                    <select class="pd_uom field_readonly moveIndex form-control erp-form-control-sm" data-id="uom" name="pd[{{$loop->iteration}}][uom]">
                                                ...
                                                <td><input readonly type="text" data-id="production_date" name="pd[{{$loop->iteration}}][production_date]" value="{{($prod_date =='01-01-1970')?'':$prod_date}}" title="{{($prod_date =='01-01-1970')?'':$prod_date}}" class="form-control form-control-sm" id="kt_datepicker_3" /></td>
                                                @php $expiry_date= date('d-m-Y', strtotime(trim(str_replace('/','-',$dtl->stock_dtl_expiry_date)))); @endphp
                                                <td><input readonly type="text" data-id="expiry_date" name="pd[{{$loop->iteration}}][expiry_date]" value="{{($expiry_date =='01-01-1970')?'':$expiry_date}}" title="{{($expiry_date =='01-01-1970')?'':$expiry_date}}" class="form-control form-control-sm" id="kt_datepicker_3" /></td>
...
@section('customJS')
    ...
    <script>
        var hiddenFieldsFormName = 'openingStockForm';
        var formcase = '{{$case}}';
    </script>
