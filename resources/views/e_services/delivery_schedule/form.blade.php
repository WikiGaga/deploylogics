...
@section('content')
    ...
    <form id="manage_schedule_form" class="manage_schedule_form kt-form" method="post" action="{{ action('EServices\DeliverScheduleController@store', isset($id) ? $id : '') }}">
    @csrf
                            ...
                            @if($case == 'edit')
                                ...
                                <tr>
                                    <td class="text-center">
                                        ...
                                        <input type="hidden" name="pd[{{ $loop->iteration }}][area_id]" autocomplete="off" value="{{ isset($current->order->area) ? $current->order->area_id : '' }}">
                                    ...
                                    <td>{{ $current->orderDtls->city->city_name }}</td>
                                    <td>{{ $current->orderDtls->area->area_name }}</td>
                                    <td>{{ $current->orderDtls->customer->customer_name }}</td>
                                    ...
                                    <td>{{ isset($current->order->sales_order_date) ? date('m-d-Y' , strtotime($current->order->sales_order_date)) : '' }}</td>
                                    <td>{{ isset($current->order->sales_order_code) ? $current->order->sales_order_code : '' }}</td>
                                    <td>{{ isset($current->order->net_total) ? $current->order->net_total : '' }}</td>
                                    <td>{{ isset($current->order->net_total) ? $current->order->net_total : '' }}</td>
                                    <td>
                                        <div class="erp-select2">
    ...
    @include('common.quick_panels.delivery_schedule')
    </form>
    <!--end::Form-->
    @endpermission
@endsection
@section('pageJS')
    ...
    <script>
        var reqURL = "{{ route('getDeliveryScheduleData') }}";
    </script>
