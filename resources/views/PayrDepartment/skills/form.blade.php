...
@section('content')
    @php
        ...
        if($case == 'new'){}
        if($case == 'edit'){
            $id = $data['current']->skill_id;
            $name = $data['current']->skill_name;
            $short_name=$data['current']->skill_short_name;
             $notes = $data['current']->skill_notes;
            $status = $data['current']->skill_entry_status;
        }
    @endphp

@permission($data['permission']);
<form id="skills_form" class="hr_department kt-form" method="post" action="{{ action('PayrDepartment\SkillsController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                ...
                <div class="kt-portlet__body">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group-block row">
                            <label class="col-lg-3 erp-col-form-label">Skill Name: <span class="required">*</span></label>
                            <div class="col-lg-6">
                        ...
                        <div class="form-group-block row">
                            <label class="col-lg-3 erp-col-form-label">Skill Short Name: </label>
                            <div class="col-lg-6">
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="skill_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="skill_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
