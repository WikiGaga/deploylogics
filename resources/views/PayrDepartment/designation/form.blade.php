...
@section('content')
    @php
        ...
        if($case == 'edit'){
            $id = $data['current']->designation_id;           
            $name = $data['current']->designation_name;
            $short_name = $data['current']->designation_short_name;
            $notes = $data['current']->designation_notes;
            $status = $data['current']->designation_entry_status;
        }
    @endphp

@permission($data['permission']);
<form id="designation_form" class="hr_department kt-form" method="post" action="{{ action('PayrDepartment\DesignationController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="designation_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="designation_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
