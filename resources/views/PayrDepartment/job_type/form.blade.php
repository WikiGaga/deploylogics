...
@section('content')
        @php
            ...
            if($case == 'new'){}
            if($case == 'edit'){
                $id = $data['current']->job_type_id;
                $name = $data['current']->job_type_name;
                $status = $data['current']->job_type_entry_status;
            }
        @endphp
    @permission($data['permission']);
<form id="job_type_form" class="hr_department kt-form" method="post" action="{{ action('PayrDepartment\JobTypeController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="job_type_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="job_type_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
