...
@section('content')
        @php
            ...
            if($case == 'new'){}
            if($case == 'edit'){
                $id = $data['current']->leave_type_id;
                $name = $data['current']->leave_type_name;
                $short_name=$data['current']->leave_type_short_name;
                $notes = $data['current']->leave_type_notes;
                $status = $data['current']->leave_type_entry_status;
            }
        @endphp
    @permission($data['permission']);
<form id="leave_type_form" class="hr_department kt-form" method="post" action="{{ action('PayrDepartment\LeaveTypeController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                ...
                <div class="kt-portlet__body">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group-block row">
                            <label class="col-lg-3 erp-col-form-label">Leave Type Name: <span class="required">*</span></label>
                            <div class="col-lg-6">
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="leave_type_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="leave_type_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
