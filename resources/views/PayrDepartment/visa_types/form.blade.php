...
@section('content')
    @php
        ...
        if($case == 'new'){}
        if($case == 'edit'){
            $id = $data['current']->visa_types_id;
            $name = $data['current']->visa_types_name;
            $status = $data['current']->visa_types_entry_status;
        }
    @endphp

@permission($data['permission']);
<form id="visa_types_form" class="hr_department kt-form" method="post" action="{{ action('PayrDepartment\VisaTypeController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="visa_types_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="visa_types_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
