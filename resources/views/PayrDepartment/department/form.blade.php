...
@section('content')
    @php
        ...
        if($case == 'new'){}
        if($case == 'edit'){
            $id = $data['current']->department_id;
            $name = $data['current']->department_name;
            $short_name=$data['current']->department_short_name;
            $notes = $data['current']->department_notes;
            $status = $data['current']->department_entry_status;
        }
    @endphp

@permission($data['permission']);
<form id="department_form" class="hr_department kt-form" method="post" action="{{ action('PayrDepartment\DepartmentController@store', isset($id)?$id:"") }}">
    @csrf
    <!-- begin:: Content -->
                ...
                <div class="kt-portlet__body">
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group-block row">
                            <label class="col-lg-3 erp-col-form-label"> Department Name: <span class="required">*</span></label>
                            <div class="col-lg-6">
                                ...
                                <span class="kt-switch kt-switch--sm kt-switch--icon">
                                    <label>
                                        @if($case == 'edit')
                                            ...
                                            <input type="checkbox" name="department_entry_status" {{$entry_status==1?"checked":""}}>
                                        @else
                                            <input type="checkbox" name="department_entry_status" checked>
                                        @endif
                                        <span></span>
                                    </label>
                                </span>
