...
@section('title', 'Product Reporting')
        ...
        <div class="row form-group-block report_list">
            ...
            @else
                @foreach($data['list'] as $list)
                    @php
                        $userStudio = \App\Models\TblSoftReportingUserStudio::where('reporting_id',$list->reporting_id)
                                                ->where('reporting_user_studio_case_name',$list->reporting_case)
                                                ->where('reporting_user_studio_user_id',auth()->user()->id)->first();
                        $userStudioId = isset($userStudio)?$userStudio->reporting_user_studio_id:"";
                    @endphp
                    <div class="col-lg-4">
                        <a href="/report/user-report/{{$list->reporting_case}}/{{$userStudioId}}" class="kt-portlet erp-wave kt-iconbox wave-primary ">
